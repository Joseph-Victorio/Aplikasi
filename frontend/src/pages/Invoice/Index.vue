              ...
              <q-item-section>
                <q-item-label>
                  <span class="q-px-md rounded-borders text-white q-py-xs"
                    :class="`bg-${getOrderStatusColor(invoice.order_status)}`">{{ invoice.customer_status_label }}</span>
                </q-item-label>
              </q-item-section>
            </q-item>
          </q-list>
          ...
          <q-card-section class="q-px-sm">
            <q-list dense>
              ...
              <q-item>
                ...
                <q-item-section>{{ invoice.shipping_courier_name }} {{ invoice.shipping_courier_service ? '' + ' - ' +
                  invoice.shipping_courier_service : '' }}</q-item-section>
              </q-item>
            </q-list>
          </q-card-section>
        </q-card>
      ...
      <q-card class="card-lg">
        <q-card-section>
          ...
          <form @submit.prevent="submitChat">
            <q-input ref="inputMessage" required type="textarea" v-model="formMessage.message" label="Isi Pesan" outlined
              rows="10"></q-input>
            <div class="q-mt-md">
