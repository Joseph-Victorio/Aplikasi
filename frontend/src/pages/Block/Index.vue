  ...
  <q-page class="q-pb-xl">
    <q-header :class="getHeaderColorBrand">
      <q-toolbar>
        ...
        <q-btn color="white" text-color="grey-8" size="13px" @click="handleAddBlock" no-caps icon="eva-plus-circle"
      ...
      <q-item v-for="block in blocks.data" :key="block.id">
        ...
        <q-item-section>
          <div class="q-mt-xs">
            ...
            <q-chip v-if="block.post_id" size="sm" icon="check_circle" outline color="teal"
    ...
    <template v-if="!blocks.available">
      <div class="text-center q-py-lg">Tidak ada data</div>
    </template>
      ...
      <q-card style="width:100%;max-width:450px;" class="bg-white">
        ...
        <form @submit.prevent="submitBlock" class="full-width">
          <q-card-section class="q-gutter-y-md">
            <q-input filled required label="Label" v-model="form.label" />
            <q-select filled v-model="form.position" :options="positionOptions" label="Pilih Tipe" />
            <q-input filled v-if="form.position == 'Featured'" label="Deskripsi" v-model="form.description" autogrow
              class="q-mb-sm" />
            <q-input filled required label="Urutan" v-model="form.weight" mask="####"
