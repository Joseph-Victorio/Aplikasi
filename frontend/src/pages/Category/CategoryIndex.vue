  ...
  <q-page :class="{ 'flex flex-center': !categories.available }">
    <q-header :class="getHeaderColorBrand">
      <q-toolbar>
        ...
        <q-btn color="white" text-color="grey-8" size="13px" class="gt-xs" label="Tambah Kategori" no-caps
          icon="eva-plus-circle" :to="{ name: 'CategoryForm' }" />
      </q-toolbar>
    </q-header>
            ...
            <template v-slot:header>
              <q-item-section avatar>
                <template v-if="cat.image">
                  <q-img :src="cat.image.src" ratio="1" class="img-thumbnail img-avatar" />
                </template>
              </q-item-section>
              <q-item-section>
                {{ cat.title }}
              ...
              <q-item-section side>
                <div class="row items-center q-gutter-x-sm">
                  ...
                  <q-btn round color="blue" dense size="11px"
                    :to="{ name: 'CategoryFormEdit', params: { category_id: cat.id } }" unelevated icon="eva-edit-2" />
                </div>
              </q-item-section>
            </template>
              ...
              <q-item v-for="item in cat.childs" :key="item.id">
                ...
                <q-item-section side>
                  <div class="text-grey-8 q-gutter-x-sm">
                    ...
                    <q-btn unelevated round color="blue" dense size="11px"
                      :to="{ name: 'CategoryFormEdit', params: { category_id: item.id } }" icon="eva-edit-2" />
                  </div>
                </q-item-section>
              </q-item>
