  ...
  <q-page class="relative" :class="{ 'flex flex-center': !ready }">
      ...
      <div class="col relative overflow-x-hidden">
        ...
        <q-card class="product-detail relative box-shadow">
          <q-card-section class="q-pt-xs q-pb-lg">
            <h1 class="text-h6 text-weight-semibold" v-if="product">
              {{ product.title }}
            </h1>
            ...
            <div>
              ...
              <div class="q-gutter-y-sm q-my-md items-center text-grey">
                <div class="text-grey text-xs">
                  Jawab tantangan berikut, hanya untuk memastikan anda bukan
                  robot
                </div>
    ...
    <q-dialog v-model="chatModal">
      <q-card style="max-width: 450px; width: 100%" class="text-grey-9">
        <div class="text-weight-bold q-py-sm q-px-md bg-primary text-white">
          Kirim pesan / tanya ke penjual
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      tab: "Description",
      defaultChat: [
        "Mohon infomasi tentang produk ini",
      ...
      imagePreviewIndex: 0,
      unapproved_review:
        JSON.parse(localStorage.getItem("unapproved_review")) || null,
      has_varian: false,
  ...
  computed: {
    product_varian_item_render() {
      if (this.product_attribute_selected) {
        return this.product.varian_items.filter(
          (v) => v.varian_id == this.product_attribute_selected.id
        );
      }
    ...
    cStyle() {
      if (
        !this.fullscreen &&
        this.$q.screen.width < 560 &&
        this.$q.screen.width > 200
      ) {
    ...
    currentStock() {
      ...
      if (this.has_varian && this.product_varian_selected) {
        return hasCart != undefined
          ? this.product_varian_selected.stock - hasCart.quantity
          : this.product_varian_selected.stock;
      } else {
        return hasCart != undefined
          ? this.productStock - hasCart.quantity
          : this.productStock;
      }
    },
    currentProductSku() {
    ...
    getDiscountAmount() {
      if (this.product.pricing.is_discount) {
        if (this.product.pricing.discount_type == "PERCENT") {
          return (
            (parseInt(this.getDefaultPrice) *
              parseInt(this.product.pricing.discount_amount)) /
            100
          );
        } else {
    ...
    getMaxPrice() {
      if (this.has_varian) {
        ...
        if (this.product_attribute_selected) {
          maxPrice = parseInt(
            this.product_varian_item_render[
              this.product_varian_item_render.length - 1
            ].price
          );
        }
  ...
  methods: {
    ...
    getcurrentDiscount(price) {
      if (this.product.pricing.is_discount) {
        if (this.product.pricing.discount_type == "PERCENT") {
          return (
            (parseInt(price) * parseInt(this.product.pricing.discount_amount)) /
            100
          );
        } else {
          return parseInt(this.product.pricing.discount_amount);
    ...
    incrementQty() {
      ...
      if (this.quantity < this.currentStock) {
        ...
        this.$q.dialog({
          title: "Warning!",
          message:
            "Stok tidak cukup, stok tersisa " + this.currentStock + " item.",
        });
      }
    },
    decrementQty() {
    ...
    getReview() {
      ...
      this.loadProductReview({ product_id: this.product.id })
        .then((response) => {
    ...
    loadReview() {
      ...
      this.loadProductReview({
        product_id: this.product.id,
        skip: this.productReviews.length,
      })
        .then((response) => {
          if (response.status == 200) {
            this.loadMoreLoading = false;
    ...
    getProduct() {
      this.productDetail(this.$route.params.slug)
        .then((response) => {
          if (response.status == 200) {
    ...
    submitChat() {
      ...
      if (!shopPhone) {
        this.$q.dialog({
          ...
          message:
            "Silahkan coba kembali beberapa saat lagi, jika masih berlanjut silahkan hubungi kami.",
        });
        return;
      }
      ...
      let link =
        ...
        this.formatPhoneNumber(shopPhone) +
        "&text=" +
