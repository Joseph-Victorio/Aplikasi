      ...
      <div :key="item.id" v-if="item.items.length" class="block-container bg-linear">
        <div class="auto-padding q-mb-xs">
          <div class="row items-end justify-between">
            ...
            <q-btn flat no-caps color="primary" padding="4px"
              :to="{ name: 'ProductCategory', params: { id: item.category_id } }">
          ...
          <div class="banner" v-if="item.banner">
            <router-link :to="{ name: 'ProductCategory', params: { id: item.category_id } }">
              <img :src="item.banner.src" :alt="item.title" class="cursor-pointer q-mt-sm" />
            </router-link>
          </div>
        </div>
        <div class="block-content">
          ...
          <div v-else class="auto-padding-side">
            <CarouselContainer :products="item.items" :loadmore="{ title: item.title, category: item.category_id }" />
          </div>
        </div>
      </div>
    </template>
