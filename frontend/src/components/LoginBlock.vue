    ...
    <q-card-section style="max-height:70vh;" class="scroll">
      <form @submit.prevent="submit" class="q-gutter-y-xs">
        ...
        <div class="column q-gutter-y-sm">
          <q-btn :loading="isLoading" unelevated type="submit" color="primary" padding="sm lg">{{ isRegister ? 'Daftar' :
            'Login' }}</q-btn>

        </div>
      </form>
      <div class="column q-gutter-y-sm text-center q-mt-xs">
        <div v-if="isRegister">Sudah punya akun <q-btn no-caps color="green-7" padding="xs" flat :disable="isLoading"
            label="Login Disini" @click="formType = 'login'"></q-btn></div>
        <div v-if="!isRegister">
          <div>
            Belum punya akun <q-btn no-caps color="green-7" padding="xs" flat :disable="isLoading" label="Daftar Disini"
              @click="formType = 'register'"></q-btn>
          </div>
          <div>
